...
export function postsReducer(state: PostState = initialState, action: Action): PostState {
  switch (action.type) {
    ...
    case PostActions.EDIT_POST_DATA_ARRIVED:
      return {...state, loading: false, selectedPost: action.payload, isEditing: true};
    ...
    case PostActions.EDIT_POST_BY_ID_SUCCESS:
      return {...state, loading: false, selectedPost: null, isEditing: false, posts: [
          ...state.posts.map(post => post.id === action.payload.id ? action.payload : post)
        ]};
    case PostActions.CANCEL_ADD_EDIT_POST_CLICKED:
      return {...state, isEditing: false, selectedPost: null}
    default:
      return state;
  }
}
